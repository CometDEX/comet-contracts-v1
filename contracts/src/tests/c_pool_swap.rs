...
fn test_swap_out_given_in() {
    ...
    // verify MAX_IN_RATIO
    let result = comet.try_swap_exact_amount_in(&token_1, &35_0000000, &token_2, &0, &i128::MAX, &user);
    ...
    assert_eq!(result.err(), Some(Ok(Error::from_contract_error(CometError::ErrNegative as u32))));
    ...
    let result = comet.try_swap_exact_amount_in(&token_1, &swap_in_amount_fixed, &token_2, &0, &over_res_price, &user);
    assert_eq!(result.err(), Some(Ok(Error::from_contract_error(CometError::ErrLimitPrice as u32))));

    // - verify limit out
    let more_than_out = float_out_fixed + 100;
    let result = comet.try_swap_exact_amount_in(&token_1, &swap_in_amount_fixed, &token_2, &more_than_out, &i128::MAX, &user);
    ...
    let (res_2_out, _) = comet
        ...
        .swap_exact_amount_in(&token_1, &swap_in_amount_fixed, &token_2, &0, &i128::MAX, &user);
    assert!(res_2_out <= float_out_fixed); // rounds down
    assert_approx_eq_rel(res_2_out, float_out_fixed, 0_0001000);

    // verify ledger state
    assert_eq!(token_1_client.balance(&user), starting_bal - swap_in_amount_fixed);
    assert_eq!(token_2_client.balance(&user), starting_bal + res_2_out);
    assert_eq!(token_1_client.balance(&comet_id), balances.get_unchecked(0) + swap_in_amount_fixed);
    assert_eq!(token_2_client.balance(&comet_id), balances.get_unchecked(1) - res_2_out);
}
...
fn test_swap_in_given_out() {
    ...
    let result = comet.try_swap_exact_amount_out(&token_2, &i128::MAX, &token_1, &36_0000000, &i128::MAX, &user);
    assert_eq!(result.err(), Some(Ok(Error::from_contract_error(CometError::ErrMaxOutRatio as u32))));

    // verify negative input
    let result = comet.try_swap_exact_amount_out(&token_2, &i128::MAX, &token_1, &-2, &i128::MAX, &user);
    assert_eq!(result.err(), Some(Ok(Error::from_contract_error(CometError::ErrNegative as u32))));
    ...
    let over_res_price = float_price_fixed + 100;
    let result = comet.try_swap_exact_amount_out(&token_2, &over_in, &token_1, &swap_out_amount_fixed, &over_res_price, &user);
    assert_eq!(result.err(), Some(Ok(Error::from_contract_error(CometError::ErrLimitPrice as u32))));

    // - verify limit it
    let less_than_in = float_in_fixed - 100;
    let result = comet.try_swap_exact_amount_out(&token_2, &less_than_in, &token_1, &swap_out_amount_fixed, &i128::MAX, &user);
    ...
    let (res_2_in, _) = comet
        ...
        .swap_exact_amount_out(&token_2, &over_in, &token_1, &swap_out_amount_fixed, &i128::MAX, &user);
    
    assert!(res_2_in >= float_in_fixed); // rounds up
    assert_approx_eq_rel(res_2_in, float_in_fixed, 0_0001000);

    // verify ledger state
    assert_eq!(token_1_client.balance(&user), starting_bal + swap_out_amount_fixed);
    assert_eq!(token_2_client.balance(&user), starting_bal - res_2_in);
    assert_eq!(token_1_client.balance(&comet_id), balances.get_unchecked(0) - swap_out_amount_fixed);
    assert_eq!(token_2_client.balance(&comet_id), balances.get_unchecked(1) + res_2_in);
}