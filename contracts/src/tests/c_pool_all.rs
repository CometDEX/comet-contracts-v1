...
fn test_pool_functions_dep_wdr() {
    ...
    token1.mint(&admin1, &admin, &i128::MAX);
    token2.mint(&admin1, &admin, &i128::MAX);

    token1.mint(&admin1, &user1, &to_stroop(40000000));
    token1.mint(&admin1, &user2, &to_stroop(40000000));
    token2.mint(&admin1, &user1, &to_stroop(40000000));
