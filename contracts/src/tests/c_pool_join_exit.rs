...
fn test_join_exit() {
    ...
    let mut balancer = BalancerPool::new(std_vec![100.0, 150.0, 50.0], std_vec![0.20, 0.50, 0.30], 0.003);
    ...
    assert_eq!(comet.get_total_supply(), starting_supply + join_amount_fixed);
    ...
    assert_approx_eq_rel(starting_bal - post_join_bal_1, in_float_fixed.get_unchecked(0), 0_0001000);
    assert_approx_eq_rel(starting_bal - post_join_bal_2, in_float_fixed.get_unchecked(1), 0_0001000);
    assert_approx_eq_rel(starting_bal - post_join_bal_3, in_float_fixed.get_unchecked(2), 0_0001000);
    ...
    // verify ratio zero
    let result = comet.try_exit_pool(&1, &below_out_fixed, &user);
    assert_eq!(result.err(), Some(Ok(Error::from_contract_error(CometError::ErrMathApprox as u32))));
    ...
    assert_eq!(comet.get_total_supply(), starting_supply + join_amount_fixed - exit_amount_fixed);
    ...
    assert!(token_1_client.balance(&comet_id) >= post_join_comet_bal_1 - out_float_fixed.get_unchecked(0));
    assert!(token_2_client.balance(&comet_id) >= post_join_comet_bal_2 - out_float_fixed.get_unchecked(1));
    assert!(token_3_client.balance(&comet_id) >= post_join_comet_bal_3 - out_float_fixed.get_unchecked(2));
    assert_approx_eq_rel(token_1_client.balance(&user) - post_join_bal_1, out_float_fixed.get_unchecked(0), 0_0001000);
    assert_approx_eq_rel(token_2_client.balance(&user) - post_join_bal_2, out_float_fixed.get_unchecked(1), 0_0001000);
    assert_approx_eq_rel(token_3_client.balance(&user) - post_join_bal_3, out_float_fixed.get_unchecked(2), 0_0001000);
}
